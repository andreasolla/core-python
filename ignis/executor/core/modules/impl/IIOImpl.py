...
class IIOImpl(IBaseImpl):
    ...
    def hdfsTextFile(self, path, minPartitions=1):
        ...
        # path: hdfs://x.x.x.x:pppp/path/to/file -> host = x.x.x.x:pppp, path = /path/to/file
        host = path.split("/")[2]
        fpath = "/" + "/".join(path.split("/")[3:])
        ...
        file = lib.Open(fpath, minPartitionSize*2, "r")
